...
- 作者❤️は「作者に届いた」という付加情報であり、別のリアクション数ではない。
...
ベルから開く画面は一般的なお知らせ一覧ではなく、クリエイターと投稿者へ反応が直接届く受信箱とする。
...
4. **運営からのお知らせ**: Moderation結果、課金・重要なアカウント情報。
...
削除済み感想は3件制限の有効件数から外れるため、投稿者が削除すると新しい感想を送れる。
...
- 作品削除も即時非公開＋30日保持とし、クライアント物理削除を許可しない。通報・Moderation証跡がある作品は自動パージしない。
...
| MOD-002 | Delete Own Feedback | 投稿者が削除 | `deleted_by_author`、画面から除外、投稿枠を戻す | 実装済み |
...
| MOD-006 | Rights Holder Request | 権利者が作品箱の削除を申請 | 公式／許諾／ファン／イベント分類と対象URLを添えて運営へ送信 | 未実装 |
...
| CHAR-002 | Companion Contact | キャラがフキダシ／キャラへ接触 | 小さいBodyで密集を許容。横へ弾かず0.15秒で縮む→膨らむ→静止 | 物理調整待ち |
...
| 運営Moderation | DB監査情報のみ | 管理画面、Status更新、異議申立て |
| アカウント削除 | 未実装 | Auth削除Function、Storage／個人情報削除導線 |
| 利用規約／Privacy | 作品登録ルールのみ | 法務文面、アプリ内リンク、同意Version管理 |
| Universal Links | アプリ解析・AASA・Associated Domains実装済み | 所有HTTPSドメイン、Webランディング、AASA配信 |
...
6. **匿名履歴の寿命と登録誘導**
   ゲスト投稿を妨げない。着地直後、または最初の反応を受け取った直後に「記録を残す」価値として登録を案内する。
...
5. 権利者向け削除申請と、公式／許諾／ファン作成／イベント分類表示を追加する。通報`MOD-001`とブロック`MOD-004`は実装済みのため作り直さない。
